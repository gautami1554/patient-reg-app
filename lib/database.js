...
export async function addPatient(patientData) {
  try {
    ...
    // Extra safety check - wait a bit more if this is likely the first operation
    if (!isInitialized) {
      console.log("⚠️ Database not fully ready, waiting...")
...
export async function deletePatient(id) {
  try {
    ...
    // CRITICAL: Force IndexedDB to persist with multiple sync attempts
    for (let syncAttempt = 0; syncAttempt < 3; syncAttempt++) {
      await new Promise((resolve) => setTimeout(resolve, 300 * (syncAttempt + 1)))

      // Force a simple query to trigger IndexedDB sync
      await database.query("SELECT 1")

      console.log(`🔄 Sync attempt ${syncAttempt + 1} completed`)
    }

    // Comprehensive verification with multiple attempts
    let verificationAttempts = 0
    let deleted = false

    while (verificationAttempts < 8 && !deleted) {
      try {
        // Wait before each verification
        await new Promise((resolve) => setTimeout(resolve, 200 * (verificationAttempts + 1)))

        // Check if patient still exists
        const verificationCheck = await database.query("SELECT id FROM patients WHERE id = $1", [id])

        if (verificationCheck.rows.length === 0) {
          deleted = true
          console.log(`✅ Deletion verified on attempt ${verificationAttempts + 1}`)
          break
        } else {
          console.log(`⚠️ Patient still exists on verification attempt ${verificationAttempts + 1}`)

          // If still exists after multiple attempts, try deleting again
          if (verificationAttempts === 3 || verificationAttempts === 6) {
            console.log(`🔄 Re-attempting deletion on verification attempt ${verificationAttempts + 1}`)
        ...
        console.warn(`Deletion verification attempt ${verificationAttempts + 1} failed:`, error)
      }
    ...
    if (!deleted) {
      throw new Error("Patient deletion verification failed after multiple attempts - record may still exist")
    }

    console.log(`✅ Patient with ID ${id} successfully deleted and verified after ${verificationAttempts} attempts`)
    return true
  } catch (error) {
